"""
compare_errors_m0040_extrapol_finalstep.py
==========================================
Load HeteroMGN (extrapol split) checkpoint, rollout on train + test,
plot 2x5 Pred vs GT scatter — FINAL STEP ONLY (t=20):
  Displacement | VM Stress Si | VM Stress Solder | VM Stress UF | PEEQ (Solder)

Extrapolation split:
  Train (184): interior UF CTE/E values
  Test  (36):  extreme boundary — all of UF E_min, E_max, CTE_min, CTE_max
               → E_norm and CTE_norm for UF are slightly outside [0,1]

UF normalization (per-material min-max for E and CTE):
  E_norm[UF]   = (E   - UF_E_min)   / (UF_E_max   - UF_E_min)
  CTE_norm[UF] = (CTE - UF_CTE_min) / (UF_CTE_max - UF_CTE_min)

Usage:
  python compare_errors_m0040_extrapol_finalstep.py    # all 184 train + 36 test
"""
...
STATS_DIR = "./04_preprocessed_hetero_m0040_extrapol"
CKPT_PATH = "./checkpoints_m0040_extrapol"
OUT_PNG   = "./compare_errors_m0040_extrapol_finalstep.png"

# Test = all 36 extreme boundary samples
TEST_INDICES = {
...
# Train = all 184 non-extreme samples
...
with open(os.path.join(STATS_DIR, "stats_m0040_extrapol.json")) as f:
    stats = json.load(f)
...
# UF-specific normalization (key for extrapolation)
UF_E_MIN   = float(stats["UF_E_min"]);   UF_E_MAX   = float(stats["UF_E_max"])
UF_CTE_MIN = float(stats["UF_CTE_min"]); UF_CTE_MAX = float(stats["UF_CTE_max"])

# ── Detect epoch ──────────────────────────────────────────────────────────────

ckpt_files = glob.glob(os.path.join(CKPT_PATH, "HeteroMGN.0.*.pt"))
latest_epoch = max(int(os.path.basename(f).split(".")[2]) for f in ckpt_files) if ckpt_files else 0
...
def rollout_sample(sid):
    ...
    # UF-specific min-max for E and CTE (matches preprocessing)
    E_norm_arr = (elem_E - E_MIN) / (E_MAX - E_MIN)
    E_norm_arr[uf_mask] = (elem_E[uf_mask] - UF_E_MIN) / (UF_E_MAX - UF_E_MIN)
    E_norm_t = E_norm_arr.astype(np.float32)[:, None]

    CTE_n_arr = (elem_CTE - CTE_mean) / CTE_std
    CTE_n_arr[uf_mask] = (elem_CTE[uf_mask] - UF_CTE_MIN) / (UF_CTE_MAX - UF_CTE_MIN)
    CTE_n_t = CTE_n_arr.astype(np.float32)[:, None]

    nu_n = ((elem_nu - nu_mean) / nu_std).astype(np.float32)[:, None]
    elem_static = np.concatenate([mat_oh, E_norm_t, CTE_n_t, nu_n], axis=1)
...
print(f"Collecting {len(TEST_IDS)} test samples (all extreme boundary) ...")
...
fig.suptitle(
    f"HeteroMGN Per-Material Norm (Extrapol Split) H256 Epoch {latest_epoch}"
    f" — Pred vs GT (final step t=20 only)",
    fontsize=20, fontweight="bold"
)

ROW_DATA  = [train_data, test_data]
ROW_LABEL = [
    f"Train — Interior UF (n={len(TRAIN_IDS)})",
    f"Test — Extreme UF boundary (n={len(TEST_IDS)})",
]
