"""
export_abaqus_paraview_nodetype.py
===================================
Run rollout with train_prevvel.py model (8 input features:
x_t(3) + prev_vel(3) + node_type_oh(2)) and export GT + predicted
results to VTU / PVD format for ParaView comparison.

Output structure:
  export_abaqus_paraview_nodetype/
    sample_XXXXX/
      gt/    t_000.vtu ...  sample_XXXXX_gt.pvd
      pred/  t_000.vtu ...  sample_XXXXX_pred.pvd

Usage:
  python export_abaqus_paraview_nodetype.py
"""
...
NUM_INPUT_FEATURES  = 8    # x_t(3) + prev_vel(3) + node_type_oh(2)
NUM_EDGE_FEATURES   = 8
...
CKPT_EPOCH = None         # None = latest; set e.g. 50 for a specific epoch
OUTPUT_DIR = "./export_abaqus_paraview_nodetype_dispnorm"

# training: indices 0~999, test: indices 2000+
...
for idx in SAMPLE_INDICES:
    ...
    # node_type_oh is constant per sample (structural property, not time-varying)
    node_type_oh = data[0]["graph"].x[:, 6:8].to(device)  # (N, 2)
    ...
    stress_vm_np   = npz_d["stress_vm"]
    gt_stress_list = [stress_vm_np[t + 1].astype(np.float32) for t in range(len(data))]

    # Autoregressive rollout — seed at t=1 with GT
    ...
    with torch.inference_mode():
        for k in range(T - 1):
            ...
            # 8 features: disp_norm(3) + prev_vel_norm(3) + node_type_oh(2)
