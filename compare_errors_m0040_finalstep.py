"""
compare_errors_m0040_finalstep.py
==================================
Load latest HeteroMGN checkpoint, rollout on train + test samples,
plot 2x5 Pred vs GT scatter — FINAL STEP ONLY (t=20):
  Displacement | VM Stress Si | VM Stress Solder | VM Stress UF | PEEQ (Solder)

vm_stress is element-level (as predicted by element nodes in HeteroMGN).

Rollout: seeded at t=1 from GT, full 20-step rollout, only final frame collected.

Usage:
  python compare_errors_m0040_finalstep.py           # 10 train + 10 test
  python compare_errors_m0040_finalstep.py 20 20     # 20 + 20
"""
...
OUT_PNG   = "./compare_errors_m0040_finalstep.png"
...
def rollout_sample(sid):
    ...
    vm_stress is element-level (element node targets in HeteroMGN).
    """
...
fig.suptitle(
    f"HeteroMGN H256 Epoch {latest_epoch} — Per-Material Pred vs GT (final step t=20 only)",
