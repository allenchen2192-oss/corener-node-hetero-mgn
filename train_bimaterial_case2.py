"""
train_bimaterial_case2.py
=========================
Case 2: Predict displacement AND stress simultaneously (4-output).

Model output: vel(3) + B-matrix_stress_vm(1)
Loss:         w_vel * MSE(vel) + w_stress * MSE(stress_direct, gt_bmat_stress)
              No physics computation — stress is a direct neural network output.

Architecture aligned with v2: 9D input, proc=4, hidden=128, no P feature.
"""
...
class Case2Trainer:
    """4-output trainer: vel(3) + direct B-matrix stress_vm(1)."""

    def __init__(self, cfg: DictConfig, rank_zero_logger):
        ...
        self.model = HybridMeshGraphNet(
            cfg.num_input_features,   # 9
            cfg.num_edge_features,    # 9
            cfg.num_output_features,  # 4
    ...
    def _load_stats(self, cfg):
        ...
        self.bmat_s_mean = torch.tensor(
            node_stats.get("bmat_stress_mean", 0.0), dtype=torch.float32, device=dev)
        self.bmat_s_std  = torch.tensor(
            node_stats.get("bmat_stress_std",  1.0), dtype=torch.float32, device=dev
        ).clamp(min=1e-6)
    ...
    def forward(self, graph, mesh_ef, world_ef):
        ...
        # Input noise on displacement (same as v2)
        if self.model.training and self.input_noise_std > 0:
            graph  = graph.clone()
            graph.x = graph.x.clone().to(dev)
            graph.x[:, :3] += (torch.randn_like(graph.x[:, :3])
                               * self.input_noise_std * self.d_std)
            x = graph.x

        # 9D node features (no P, same as v2)
        node_x = torch.cat([
        ...
        with autocast("cuda", dtype=torch.bfloat16, enabled=self.amp):
            ...
            # pred: (N, 4) = [vel_norm(3), stress_vm_norm(1)]

        # Velocity loss (same as v2)
        gt_vel   = graph.y[:, :3].to(dev).float()
        loss_vel = self.criterion(pred[:, :3].float(), gt_vel)

        # Direct stress loss (predict B-matrix VM stress as a 4th output)
        gt_bmat_norm = ((graph.bmat_stress.to(dev).float() - self.bmat_s_mean)
                        / self.bmat_s_std)
        loss_stress  = self.criterion(pred[:, 3].float(), gt_bmat_norm)

        loss = self.w_vel * loss_vel + self.w_stress * loss_stress
...
@hydra.main(version_base="1.3", config_path="conf", config_name="config_bimaterial_case2")
def main(cfg: DictConfig) -> None:
    ...
    trainer = Case2Trainer(cfg, rank_zero_logger)
    rank_zero_logger.info("Training started... [Case 2: vel(3) + direct stress(1)]")
    ...
    for epoch in range(trainer.epoch_init, cfg.epochs):
        ...
        for batch in progress_bar:
            ...
            loss, loss_vel, loss_stress = trainer.train_step(
                batched_graph, batched_mesh_ef, batched_world_ef
            )
        ...
        if dist.rank == 0:
            ...
            trainer.writer.add_scalar("train/loss_vel",    loss_vel.item(),     epoch)
            trainer.writer.add_scalar("train/loss_stress", loss_stress.item(),  epoch)
            rank_zero_logger.info(
                f"epoch: {epoch + 1}, loss: {mean_loss:.3e}, "
                f"vel: {loss_vel.item():.3e}, stress: {loss_stress.item():.3e}, "
                f"time: {elapsed:.1f}s"
            )
            save_checkpoint(
                to_absolute_path(cfg.ckpt_path),
