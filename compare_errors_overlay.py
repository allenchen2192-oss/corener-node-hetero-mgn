"""
compare_errors_overlay.py
=========================
Overlay rollout error curves: with vs without stress supervision.

  w_stress=0.5 : _dispnorm checkpoint, with GFDM stress loss
  w_stress=0   : _no_stress checkpoint, velocity loss only

Usage:
  python compare_errors_overlay.py
"""
...
OUTPUT_PLOT = "./compare_errors_overlay_no_nodetype.png"
...
def run_sample(fname, model, normalized, use_node_type=True):
    ...
    with torch.inference_mode():
        for _ in range(T - 1):
            ...
            if normalized:
                ...
                disp_part = cur - ref_pos          # raw
                vel_part  = prev_vel               # raw
...
def collect(fnames, model, normalized, label, use_node_type=True):
...
for cfg_name, cfg in CONFIGS.items():
    print(f"\n=== {cfg_name} ({cfg['ckpt'].split('/')[-1]}) ===")
    n_in = cfg.get("num_input_features", NUM_INPUT_FEATURES)
    ...
    norm          = cfg["normalized"]
    use_node_type = cfg.get("use_node_type", True)
    train_d, train_s = collect([all_files[i] for i in train_indices], model, norm, f"{cfg_name}/train", use_node_type)
    test_d,  test_s  = collect([all_files[i] for i in test_indices],  model, norm, f"{cfg_name}/test",  use_node_type)
    results[cfg_name] = {
...
style = {
    cfg_keys[0]: {"ls": "-",  "marker": "o", "ms": 4, "markevery": 2},   # w_stress=0.5: circle
    cfg_keys[1]: {"ls": "--", "marker": "s", "ms": 4, "markevery": 2},   # w_stress=0  : square
}
color = {"train": "tab:blue", "test": "tab:green"}

fig, axes = plt.subplots(1, 2, figsize=(14, 5))
fig.suptitle(
    f"Stress Supervision: w_stress=0.5 vs w_stress=0  (mean over {NUM_TRAIN_SAMPLES} samples)",
    fontsize=16
)

for ax, (ylabel, key) in zip(axes, [("Displacement Error (%)", "d"), ("Stress Error (%)", "s")]):
    for cfg_name in cfg_keys:
        ls = style[cfg_name]["ls"]
        for split in ["train", "test"]:
            arr = results[cfg_name][f"{split}_{key}"]
            mu  = np.nanmean(arr, axis=0)
            c   = color[split]
            label = f"{cfg_name} – {'Training' if split=='train' else 'Test'}"
            ax.plot(ts, mu, color=c, linestyle=ls, linewidth=2, label=label,
                    marker=style[cfg_name]["marker"],
                    markersize=style[cfg_name]["ms"],
                    markevery=style[cfg_name]["markevery"])
