"""
compare_errors_m0040_cornernode_finalstep.py
=============================================
...
Rollout: seeded at t=1 from GT, full 20-step rollout, only final frame collected.
...
OUT_PNG   = "./compare_errors_m0040_cornernode_finalstep.png"
...
def rollout_sample(sid):
    ...
    solder_node_mask = sol_node   # alias used for PEEQ rollout state
    ...
    for fi in range(1, T + 1):   # fi = 1..20
        ...
        if fi == T:
            ...
            out["vm_gt_Si"].append(gt_vm_node[si_node])
            out["vm_pred_Si"].append(vm_node_phys[si_node])
            out["vm_gt_Sl"].append(gt_vm_node[sol_node])
            out["vm_pred_Sl"].append(vm_node_phys[sol_node])
            out["vm_gt_UF"].append(gt_vm_node[uf_node])
            out["vm_pred_UF"].append(vm_node_phys[uf_node])
...
fig.suptitle(
    f"Corner-Node MGN  H256 Epoch {latest_epoch} — Pred vs GT (final step t=20 only)",
