...
"""
export_predictions_paraview.py
===============================
Run model rollout on selected samples and export both GT and predicted
results to VTU / PVD format for side-by-side comparison in ParaView.

Output structure:
  export_pred_paraview/
    sample_XXXXX/
      gt/
        t_000.vtu  t_001.vtu  ...    ← ground truth
        gt.pvd
      pred/
        t_000.vtu  t_001.vtu  ...    ← model prediction
        pred.pvd
...
Fields per node:
  ...
  disp_mag       (scalar)
  von_mises      (scalar, Pa)
  disp_error     (scalar, pred only — ||pred_pos - gt_pos||)

Usage:
  Edit SAMPLE_INDICES and CKPT_EPOCH at the bottom, then run:
    python export_predictions_paraview.py
"""
...
CKPT_PATH   = "./checkpoints_nonlinear_analytical_wstress01"
CKPT_EPOCH  = None     # None = latest; set e.g. 3000 for a specific epoch
OUTPUT_DIR  = "./export_pred_paraview"
N_GRID      = 4        # 4×4×4 mesh
...
def make_graph(wp):
    ...
    return Data(x=wp - ref_pos, edge_index=mesh_ei, edge_attr=ef), ef
