...
#
# Direct stress prediction version:
#   Model outputs velocity (3) + stress (1) simultaneously.
#   No analytical stress computation.
#   Use with: python train_direct_stress.py --config-name=config_direct_stress
import os
import random
import time
...
# ─────────────────────────────────────────────────────────────────────────────
# Datasets (identical to train.py)
# ─────────────────────────────────────────────────────────────────────────────
...
class InMemorySequenceDataset(torch.utils.data.Dataset):
    def __init__(self, sample_dir, num_time_steps, seq_len, num_samples=None):
        ...
        for f in sample_files:
            all_data.append(torch.load(f, map_location="cpu", weights_only=False))
        self.sequences = [
            (si, start)
...
# ─────────────────────────────────────────────────────────────────────────────
# Trainer — direct stress prediction
# ─────────────────────────────────────────────────────────────────────────────
...
class MGNTrainer:
    def __init__(self, cfg: DictConfig, rank_zero_logger: RankZeroLoggingWrapper):
        ...
        if seq_len > 1:
            ...
            dataset = InMemoryTimeStepDataset(
                sample_dir, num_samples=cfg.num_training_samples,
            )
    ...
    def _make_node_x(self, wp, ref_pos):
        return wp - ref_pos
    ...
    def _rebuild_mesh_ef(self, wp, mesh_ei, ref_feat_norm):
        ...
        return torch.cat([ref_feat_norm, world_feat_n], dim=1)
    ...
    def train(self, graph, mesh_edge_features, world_edge_features, epoch):
        ...
        loss, loss_vel, loss_stress = self.forward(graph, mesh_edge_features, world_edge_features)
        self.backward(loss)
        self.scheduler.step()
    ...
    def forward_rollout(self, sequence, epoch):
        ...
        for k, item in enumerate(sequence):
            ...
            with torch.no_grad():
                vel_pred = pred[:, :3].float() * self.v_std + self.v_mean
...
@hydra.main(version_base="1.3", config_path="conf", config_name="config_direct_stress")
