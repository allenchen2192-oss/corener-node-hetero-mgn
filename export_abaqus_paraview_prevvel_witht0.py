"""
export_abaqus_paraview_prevvel_witht0.py
=========================================
Run rollout with the prevvel (w_stress=0, with t=0) model and export GT + predicted
results to VTU / PVD format for side-by-side comparison in ParaView.

Model: checkpoints_abaqus_prevvel_1000data_1000nodes_h128_dispnorm_no_nodetype_witht0
  Input  : disp_vec(3) + prev_vel(3) = 6D
  Output : velocity(3) = 3D

Stress in output is computed from predicted positions via GFDM (scatter linear fit).

Output structure:
  export_abaqus_paraview_prevvel_witht0/
    sample_XXXXX/
...
PointData fields:
  GT:
    ...
    von_mises_computed  (scalar, Pa)  — GFDM scatter linear fit

  Pred:
    displacement        (vector, m)
    disp_mag            (scalar, m)
    von_mises_computed  (scalar, Pa)  — GFDM from predicted positions
    disp_error          (scalar, m)   — ||pred_pos - gt_pos||

Usage:
  python export_abaqus_paraview_prevvel_witht0.py
"""
...
NUM_EDGE_FEATURES   = 8
NUM_OUTPUT_FEATURES = 3    # velocity(3) only
PROCESSOR_SIZE      = 4
HIDDEN_DIM          = 128

DATA_DIR   = "./04_preprocessed_pt_abaqus_prevvel"
CKPT_PATH  = "./checkpoints_abaqus_prevvel_1000data_1000nodes_h128_dispnorm_no_nodetype_witht0"
CKPT_EPOCH = None          # None = latest; set e.g. 500 for a specific epoch
OUTPUT_DIR = "./export_abaqus_paraview_prevvel_witht0"
